...
(function (){
    ...
    async function clickCopyCredit() {
        ...
        try {
            ...
            const latest_judge_records = await getLatestJudgeRecords()
            .catch((err) => {
                console.log(err);
            ...
            const updated_count = await updateKomutenCredits(latest_judge_records)
            .catch((err) => {
                console.log(err);
    ...
    async function getLatestJudgeRecords() {
        ...
        const all_judge = await kintoneRecord.getAllRecordsByQuery(body_judges);

        // 重複なしの取引企業Noを取得
        const unique_customer_codes = all_judge.records
            .map((record) => record[customerCode_JUDGE]['value'])
            .filter((code, i, self) => self.indexOf(code) === i);

        const latest_judges = unique_customer_codes.map((customer_code) => {
            // 取引企業Noごとにfilterして処理する
            const target_judges = all_judge.records.filter(record => record[customerCode_JUDGE]['value'] === customer_code);
            ...
            return target_judges.find(record => record[judgedDay_JUDGE]['value'] === latest_yyyy_mm_dd);
        });

        return latest_judges;
    }
    ...
    async function updateKomutenCredits(latest_judge_records) {
        ...
        const komuten_info = await kintoneRecord.getAllRecordsByQuery(body_judges);
