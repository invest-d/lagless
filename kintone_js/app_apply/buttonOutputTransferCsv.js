...
(function() {
    ...
    kintone.events.on("app.record.index.show", (event) => {
        if (common_logics.needToShow(event, realtor.buttonName, realtor_logics.AVAILABLE_VIEW)) {
            const button = realtor.createButton();
            kintone.app.getHeaderMenuSpaceElement().appendChild(button);
        }

        if (common_logics.needToShow(event, wfi.buttonName, wfi_logics.AVAILABLE_VIEW)) {
