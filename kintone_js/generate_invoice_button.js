...
(function() {
    ...
    kintone.events.on('app.record.index.show', (event) => {
        // ボタンを表示するか判定
        if (needShowButton()) {
            var button = getGenerateInvoiceButton();
    ...
    function getGenerateInvoiceButton() {
        var generateInvoice = document.createElement('button');
    ...
    function clickGenerateInvoice() {
        var clicked_ok = confirm('支払実行済みのレコードに対して、振込依頼書を作成しますか？');
        if (!clicked_ok) {
            alert('処理は中断されました。');
            return;
        }
        ...
        return new kintone.Promise((resolve, reject) => {
            ...
            .then((detail_records) => {
                ...
                paid_records.map((paid_record) => {
                    var detail_arr = [];
                    // 回収IDフィールドが一致するものを結合
                    var summary_id = paid_record[fieldRecordId_COLLECT]['value'];
                    detail_records.map((detail_record) => {
                        var detail_id = detail_record[fieldCollectId_APPLY]['value'];
    ...
    function generateInvoices(invoice_objects) {
        return new kintone.Promise((resolve, reject) => {
            ...
            invoice_objects.map((obj) => {
                var komuten_id = obj[fieldConstructionShopId_COLLECT]['value'];
                var deadline = obj[fieldDeadline_COLLECT]['value'];
                ...
                if ((komuten_id in key_pair) && (key_pair[komuten_id] === deadline)) {
                    ...
                    var dpl_invoice = combined_invoices.filter((combined) => {
            ...
            combined_invoices.map((invoice_obj) => {
                var invoice_text = generateInvoiceText(invoice_obj);
                var bom  = new Uint8Array([0xEF, 0xBB, 0xBF]);
                var blob = new Blob([bom, invoice_text], {"type": "text/plain"});

                // 作成した振込依頼書のダウンロードリンクを設置
                var file_name = invoice_obj[fieldConstructionShopName_COLLECT]['value'] + '様向け' + invoice_obj[fieldDeadline_COLLECT]['value'] + '回収振込依頼書';
    ...
    function generateInvoiceText(invoice_obj) {
        ...
        var product_name = invoice_obj[fieldProductName_COLLECT]['value'];
        lines.push(`${product_name} 振込依頼書 兼 支払明細書`);

        var date = formatYYYYMD(invoice_obj['details'][0][fieldPayDate_APPLY]['value']);
        lines.push(`\r\n${date}`);

        var company = invoice_obj[fieldConstructionShopName_COLLECT]['value'];
        lines.push(`\r\n${company}`);

        var title = invoice_obj[fieldCeoTitle_COLLECT]['value'];
        var ceo = invoice_obj[fieldCeoTitle_COLLECT]['value'];
...
敬具`);
        ...
        var closingDate = formatYYYYMD(invoice_obj[fieldClosingDate_COLLECT]['value']);
        lines.push(`\r\n対象となる締め日 ${closingDate}`);

        var deadline = formatYYYYMD(invoice_obj[fieldDeadline_COLLECT]['value']);
        lines.push(`お支払期限 ${deadline}`);

        var collectable_amount = Number(invoice_obj[fieldCollectableAmount_COLLECT]['value']).toLocaleString();
        lines.push(`ご請求金額（消費税込み） ${collectable_amount}円`);
        lines.push('￣￣￣￣￣￣￣￣￣￣￣￣' + '￣'.repeat(Math.ceil(collectable_amount.length / 2) + 1) + '￣');

        var account_id = invoice_obj[fieldAccount_COLLECT]['value'];
        var account = '';
        switch(account_id) {
            case 'ID':
        ...
        invoice_obj['details'].map((detail) => {
            var payment_date_detail = formatYYYYMD(detail[fieldPayDate_APPLY]['value']);
            var payment_amount_detail = Number(detail[fieldReceivables_APPLY]['value']).toLocaleString();
        ...
        var mail = invoice_obj[fieldMailToInvest_COLLECT]['value'];
        lines.push(`
【${product_name}事務局】
インベストデザイン株式会社
東京都千代田区一番町15番地1
...
TEL:0120-516-818`);
    ...
    function formatYYYYMD(yyyy_mm_dd) {
        // Numberを使ってゼロ埋めされているのを取り除く
        var date = String(yyyy_mm_dd).split('-');
